...
using namespace std;
struct struct_person // описываем структуру struct_person включающую: 
{
...
int N; // количество участвующих в конкурсе
int local_year=0; // текущий год
// Инициализируем массив участников с ФИО 
char name[5][60]={
...
cin>>N;
struct_person *person = new struct_person[N]; // создаем массив структур типа struct_person
...
int year_low=0, year_high=0; // нижний верхний предел возрастной группы
int stud_count=0; // количество участников попавших в возрастную группу
int stud[10]; // используется для запоминания попал ли участник в возрастную группу или нет
...
struct_person *person_grup = new struct_person[stud_count]; // создаем массив структур типа struct_person зазмером stud_count
