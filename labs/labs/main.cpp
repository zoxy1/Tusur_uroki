...
//stud_count=10-stud; // колличество студентов которые не попали в первую очередь
struct_student *stud2 = new struct_student[stud_count];
int k=0;
//struct_student student2[stud_count]; 
for(int i=0;i<10;i++)
	{
		if(stud[i]==2)
		{
 ...
 for (int i = 0; i < stud_count; i++)
    {
        for (int j = stud_count - 1; j > i; j--)
        {
            ...
            {
				swap (stud2[j].bal, stud2[j - 1].bal);
            }
        }
    }
...
for(int i=0;i<stud_count;i++)
{
	cout<<"Средний бал:"<<stud2[i].bal<<stud2[i].FIO<<endl;
}
