...
switch(znak) //в зависимости от введенного символа выполняем арифметическое действие
{
	...
	break;
	case '*':
	cout<<"a*b="<<calculate_ptr->umnogit()<<endl; //выдаст ошибку, т к метод umnogit() находится в разделе private класса и недоступен за пределами класса
	break;
	case '/':
	cout<<"a/b="<<calculate_ptr->delity()<<endl; //выдаст ошибку, т к не метод delity() находится в разделе protected класса calculate
	// доступ к методу delity() можно получить только через производный класс от класса calculate
	break;
